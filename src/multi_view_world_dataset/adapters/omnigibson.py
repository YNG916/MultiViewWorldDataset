...
from multi_view_world_dataset.sampling.trajectories import sample_smooth_trajectory_set, trajectory_kinematic_metrics
...
class OmniGibsonAdapter(BaseSimulatorAdapter):
    ...
    def sample_robot_trajectories(self, seed: int) -> tuple[tuple[Trajectory, ...], dict[str, Any]]:
        ...
        padded = np.pad(eroded_native, pixel_tolerance, constant_values=False)
        tolerant_traversable = np.zeros_like(eroded_native)
        height, width = eroded_native.shape
        for row_offset in range(2 * pixel_tolerance + 1):
            for column_offset in range(2 * pixel_tolerance + 1):
                tolerant_traversable |= padded[
        ...
        def is_path_traversable(path_xy: np.ndarray) -> bool:
            ...
            columns = np.trunc(
                points[:, 0] / float(trav_map.map_resolution)
                + float(trav_map.map_size) / 2.0
            ).astype(int)
            rows = np.trunc(
                points[:, 1] / float(trav_map.map_resolution)
                + float(trav_map.map_size) / 2.0
            ).astype(int)
            inside = (
                (rows >= 0)
            ...
            return bool(
                np.all(inside)
                and np.all(tolerant_traversable[rows, columns])
            )
        ...
        trajectories = sample_smooth_trajectory_set(
            ...
            maximum_attempts=int(self.config["placement"]["maximum_attempts"]),
        )
        metrics = {
            "floor_index": floor_index,
            "traversable_candidate_count": int(len(world_xy)),
            "robots": {trajectory.robot_id: trajectory_kinematic_metrics(trajectory) for trajectory in trajectories},
